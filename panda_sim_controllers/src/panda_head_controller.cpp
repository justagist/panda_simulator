...
{
const double PandaHeadController::HEAD_PAN_DEADBAND_RADIANS = 0.01;
const double PandaHeadController::HEAD_PAN_DEADBAND_VELOCITY = HEAD_PAN_DEADBAND_RADIANS * 2;

PandaHeadController::PandaHeadController() : new_command(true), update_counter(0)
{
}

PandaHeadController::~PandaHeadController()
{
  head_command_sub.shutdown();
}

bool PandaHeadController::init(panda_hardware_interface::SharedJointInterface* robot, ros::NodeHandle& nh)
{
  // Store nodehandle
  nh_ = nh;
  ...
  ROS_ASSERT_MSG(n_joints == 1, "PandaHeadController only functions with one joint. Exiting.");
  ROS_INFO_STREAM_NAMED("head", "Initializing PandaHeadController with " << n_joints << " joints.");
  for (auto joint_it : xml_struct)
  {
    ...
    // Get the joint-namespace nodehandle
    {
      ...
      head_controller_ptr = std::make_shared<panda_effort_controllers::JointPositionController>();
  ...
  // Update at 100Hz
  head_state_pub_timer_ = nh.createTimer(100, &PandaHeadController::publishHeadState, this);
  return true;
}
...
void PandaHeadController::starting(const ros::Time& time)
{
...
void PandaHeadController::stopping(const ros::Time& time)
{
}

void PandaHeadController::update(const ros::Time& time, const ros::Duration& period)
{
  update_counter++;
...
void PandaHeadController::updateCommands()
{
...
PLUGINLIB_EXPORT_CLASS(panda_sim_controllers::PandaHeadController, controller_interface::ControllerBase)
